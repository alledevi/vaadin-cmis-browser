...
public class SearchView extends VerticalLayout implements View  {
	...
	public SearchView() {
		...
		nameCb = new ComboBox(null,map.keySet());
		...
		nameCb.addListener(new Listener() {
			...
			public void componentEvent(Event event) {
				selectedValue = nameCb.getValue().toString();	// selectedvalue non da il path!!
				client.navigateTo(map.get(selectedValue));	// ATTENZIONE!! SE SI SELEZIONA UN DOCUMENTO NON VA!
//				System.out.println(client.getCurrentFolder().getPath());
	...
	public void fillName(ComboBox c, String path){			
		...
		for(FileView file : files){
			...
			System.out.println(map.get(file.getName()));
			if(file.isDocument()){
